...
as $$
  select jsonb_build_object(
    ...
    'requestedSessionDate', reference.requested_session_date::text,
...
begin
  ...
  if p_stage = 'S1_DISPOSITION' then
    v_requested_session_date := v_round.s1_session_date;
    v_stage_available_at := v_round.s1_close_available_at;
    v_stage_deadline_at := v_round.s2_open_at;
    if v_effective_date > v_requested_session_date then
      raise exception 'S1 disposition FX date follows the Round session'
        using errcode = '22023';
    end if;
  else
    v_requested_session_date := v_round.s2_session_date;
    v_stage_available_at := v_round.cycle_ready_at;
    v_stage_deadline_at := null;
    if v_effective_date > v_requested_session_date then
      raise exception 'S2 acquisition FX date follows the Round session'
        using errcode = '22023';
    end if;
  end if;
  ...
  if found then
    if v_existing.idempotency_key is distinct from p_idempotency_key
      ...
      or v_existing.requested_session_date is distinct from v_requested_session_date
  ...
  insert into public.arena_round_tax_fx_reference (
    ...
    raw_body_sha256, requested_session_date, effective_date,
    ...
    p_source_artifact_sha256, p_raw_body_sha256, v_requested_session_date,
    v_effective_date,
