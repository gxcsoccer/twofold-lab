-- One general, content-addressed persistence boundary for a complete accepted
-- target cycle. Financial derivation stays in the versioned Core; Postgres
-- verifies identity, exact bytes, plan bindings, replay shape, CAS ordering,
-- and atomically publishes the immutable artifact plus its read projection.

begin;

create table public.accepted_target_cycle (
  ...
  s2_frozen_order_plan_id uuid not null,
  artifact_schema text not null
    check (artifact_schema = 'twofold.accepted_target_cycle/v1'),
  cycle_canonical_json text not null check (cycle_canonical_json <> ''),
  cycle jsonb not null,
  cycle_sha256 text not null unique check (cycle_sha256 ~ '^[0-9a-f]{64}$'),
  completed_at timestamptz not null,
  source_event_id uuid not null unique references public.event_stream(event_id),
  source_stream_seq bigint not null check (source_stream_seq > 0),
  recorded_by text not null check (recorded_by <> ''),
  recorded_at timestamptz not null default clock_timestamp(),
  constraint accepted_target_cycle_account_run_fk foreign key (
    strategy_account_id,
    run_id
  ) references public.strategy_account(strategy_account_id, run_id),
  constraint accepted_target_cycle_submission_fk foreign key (
    accepted_submission_id
  ) references public.accepted_target_submission(submission_id),
  constraint accepted_target_cycle_s1_plan_fk foreign key (
    s1_frozen_order_plan_id,
    strategy_account_id
  ) references public.frozen_order_plan(frozen_order_plan_id, strategy_account_id),
  constraint accepted_target_cycle_s2_plan_fk foreign key (
    s2_frozen_order_plan_id,
    strategy_account_id
  ) references public.frozen_order_plan(frozen_order_plan_id, strategy_account_id),
  constraint accepted_target_cycle_payload_object check (
    jsonb_typeof(cycle) = 'object'
  ),
  constraint accepted_target_cycle_payload_decimal_safe check (
    not public.jsonb_contains_number(cycle)
  ),
  constraint accepted_target_cycle_payload_exact check (
    cycle_canonical_json::jsonb = cycle
  ),
  constraint accepted_target_cycle_hash_exact check (
    cycle_sha256 = encode(
      extensions.digest(convert_to(cycle_canonical_json, 'UTF8'), 'sha256'),
      'hex'
    )
  ),
  constraint accepted_target_cycle_deterministic_id check (
...
comment on table public.accepted_target_cycle is
  'Immutable Core-derived accepted target -> S1 -> S2 -> ledger -> NAV replay artifact. The database admits exact bytes and invariants; it does not duplicate financial algorithms.';

create index accepted_target_cycle_run_completed_idx
  on public.accepted_target_cycle(run_id, completed_at desc, cycle_id);

create trigger accepted_target_cycle_is_immutable
before update or delete on public.accepted_target_cycle
for each row execute function public.reject_immutable_mutation();

create trigger accepted_target_cycle_rejects_truncate
before truncate on public.accepted_target_cycle
for each statement execute function public.reject_immutable_mutation();

create or replace function public.accepted_target_cycle_commit_result(
  p_cycle public.accepted_target_cycle
)
returns jsonb
language sql
stable
...
as $$
  select jsonb_build_object(
    ...
    'strategyAccountId', p_cycle.strategy_account_id::text,
    'runId', p_cycle.run_id::text,
    'decisionId', p_cycle.decision_id::text,
    'acceptedSubmissionId', p_cycle.accepted_submission_id::text,
    's1FrozenOrderPlanId', p_cycle.s1_frozen_order_plan_id::text,
    's2FrozenOrderPlanId', p_cycle.s2_frozen_order_plan_id::text,
    'cycleSha256', p_cycle.cycle_sha256,
    'sourceEventId', p_cycle.source_event_id::text,
    'sourceStreamSeq', p_cycle.source_stream_seq::text,
    'projectionName', 'dashboard.accepted_target_cycle',
    'recordedBy', p_cycle.recorded_by,
    'recordedAt', to_char(
      p_cycle.recorded_at at time zone 'UTC',
...
begin
  ...
  if found then
    if v_existing.idempotency_key is distinct from p_idempotency_key
      ...
      or v_existing.source_stream_seq is distinct from p_expected_run_stream_seq + 1
  ...
  if jsonb_typeof(v_cycle#>'{s1,plan}') is distinct from 'object'
    ...
    or v_cycle#>>'{s1,plan,planFingerprint}'
...
comment on function public.commit_accepted_target_cycle(
  ...
  'Atomically admits a content-addressed Core cycle after both frozen plans exist, appends the run event under CAS, and publishes its decision-scoped NAV projection.';

alter table public.accepted_target_cycle enable row level security;

revoke all on table public.accepted_target_cycle
  from public, anon, authenticated, service_role;
revoke all on function public.accepted_target_cycle_commit_result(
  public.accepted_target_cycle
) from public, anon, authenticated, service_role;
revoke all on function public.commit_accepted_target_cycle(
  text, uuid, uuid, uuid, uuid, uuid, uuid, uuid, text, text, timestamptz,
  bigint, bigint, uuid, text
) from public, anon, authenticated, service_role;

grant execute on function public.commit_accepted_target_cycle(
  text, uuid, uuid, uuid, uuid, uuid, uuid, uuid, text, text, timestamptz,
  bigint, bigint, uuid, text
) to service_role;
