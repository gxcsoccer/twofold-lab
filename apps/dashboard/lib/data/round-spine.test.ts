...
import { deriveRoundSpine, isDeadlineBreach, nextRoundBoundary } from "./round-spine";
...
function overview(
  ...
  stage: PrivateArenaOverview["currentRound"] extends null ? never
    : NonNullable<PrivateArenaOverview["currentRound"]>["stage"] = "S2_EXECUTION",
): PrivateArenaOverview {
  return {
...
describe("round spine", () => {
  ...
  it("names the next frozen boundary for each stage", () => {
    ...
    expect(nextRoundBoundary({ ...round, stage: "DECISION_WINDOW" }))
      .toEqual({ label: "决策截止", at: round.decisionWindowClosesAt });
    expect(nextRoundBoundary({ ...round, stage: "S2_EXECUTION" }))
      .toEqual({ label: "S2 收盘", at: round.s2CloseAt });
    expect(nextRoundBoundary({ ...round, stage: "COMPLETE" })).toBeNull();
  });
